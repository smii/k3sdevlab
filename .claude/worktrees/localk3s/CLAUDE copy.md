...
|-----|-----|-----------|------|
| **Homarr Portal** | https://portal.rtm.kubernative.io | homarr | OIDC (own) |
...
| Coder | https://realcoder.rtm.kubernative.io | coder | OIDC + forward-auth |
...
│                                                                 │
│  ┌─────────┐  ┌─────────┐  ┌─────────┐  ┌─────────────────┐  │
│  │ Homarr  │  │  Gitea  │  │ Harbor  │  │  Coder (remote  │  │
│  │ portal  │  │  (git)  │  │(registry│  │   dev env)      │  │
│  │homarr ns│  │gitea ns │  │harbor ns│  │   coder ns      │  │
│  └─────────┘  └─────────┘  └─────────┘  └─────────────────┘  │
│  ┌─────────┐  ┌─────────┐  ┌─────────┐  ┌─────────────────┐  │
...
| `homarr` | Homarr Portal | `https://portal.rtm.kubernative.io/api/auth/callback/oidc` |

Secrets are stored in `charts/authelia/authelia-values.yaml` — do not duplicate here.

> **Homarr**: Does NOT use `authelia-forwardauth` middleware on its ingress.
> Homarr handles OIDC via NextAuth.js internally. Add `bypass` ACL rule for `portal.rtm.kubernative.io`
> in Authelia so the forward-auth middleware from the wildcard rule doesn't interfere.
> The `no groups found in profile` warning in Homarr logs is non-fatal — user is still authenticated.
> Homarr 0.16.0 config (`default.json`) requires `settings.customization.colors.{primary,secondary,shade}`
> or the page throws `TypeError: Cannot read properties of undefined (reading 'colors')` in SSR.
...
| Homarr colors SSR crash | `default.json` must have `settings.customization.colors.{primary,secondary,shade}`. Missing fields cause `TypeError: Cannot read properties of undefined (reading 'colors')` in Next.js SSR. |
| Homarr no groups warning | `WARN no groups found in profile of oidc user` is non-fatal. User is still authenticated. Homarr uses groups only for admin/owner role assignment. |
| Homarr icons `/icons/` path | Files copied to `/app/public/icons/` in the pod are NOT served via HTTP (likely Next.js build-time static path exclusion). Use external URLs only for icon `iconUrl` fields. |
...
### Homarr — Homelab Portal Dashboard

**Installed by default** — `argocd/applications/apps/homarr.yaml`

| Item | Value |
|------|-------|
| Namespace | `homarr` |
| URL | `https://portal.rtm.kubernative.io` |
| Helm repo | `https://oben01.github.io/charts/` chart `homarr` |
| Version | `1.*` (image: `ghcr.io/ajnart/homarr:0.16.0`) |
| SSO | Authelia OIDC via NextAuth.js (client_id `homarr`) |
| Config | `/app/data/configs/default.json` on `homarr-config` PVC |

**Dashboard config** — 14 service tiles + 3 widgets (date, search, weather):
- Config generated via Python and stored in the PVC at `/app/data/configs/default.json`
- Icons: walkxcode CDN for most; simpleicons for ArgoCD; GitHub org avatars for Tekton/Falco
- Status checker enabled on all tiles (200/301/302/401/403 accepted)

**Do NOT add `authelia-forwardauth@kubernetescrd`** to the Homarr ingress — Homarr handles OIDC itself. Double auth causes redirect loops.

**Required config structure** (`settings.customization.colors`):
```json
"settings": {
  "customization": {
    "colors": { "primary": "blue", "secondary": "indigo", "shade": 6 }
  }
}
```
...
| Database | `postgres:16-alpine` StatefulSet at `k8s-manifests/coder-postgres/` (NOT Bitnami) |
| Wildcard | `*.realcoder.rtm.kubernative.io` — A record required at TransIP for workspace port forwarding |

**Gotchas:**
- Workspace templates (Docker, K8s) are configured inside the Coder UI after first login
- Both `realcoder.rtm.kubernative.io` AND `*.realcoder.rtm.kubernative.io` must have A records at TransIP
- DB password and OIDC secret are in `charts/coder/coder-values.yaml`
- Coder Service type is LoadBalancer (K3s assigns nodePort/clusterIP) — `ignoreDifferences` in ArgoCD app suppresses the drift
...
| Homarr | 10m/500m | 32Mi/768Mi | `charts/homarr/homarr-values.yaml` |
